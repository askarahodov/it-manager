...
- [ ] Hosts: в фильтрах часть подписей на английском (“Tag key/value”, “Hostname”, “User”) — ломает консистентность локализации.
- [ ] Sort UX: кликабельные заголовки таблиц без `aria-sort` и без явного affordance (кроме стрелок) — хуже A11y.
- [ ] Automation: длинная форма без “быстрой навигации” и/или sticky action bar (сохранить/сброс) — много скролла.
- [ ] Ошибки форм: нет единообразных inline ошибок под полями (местами только toast/общая ошибка).
- [ ] Loading: нет skeleton/placeholder для таблиц (скачки layout при загрузке).

### P2 (можно улучшать дальше)

- [ ] Контраст helper-текста на тёмной теме местами на грани читабельности.
- [ ] Единый стиль иконок/кнопок (например “Меню” как текст vs иконка) — косметика.
- [ ] Табличные ячейки с длинными строками иногда слишком агрессивно переносятся (влияние на читабельность).
- [ ] Подсказки (title) не всегда заменяют явные подсказки в UI (особенно для touch).
- [ ] Нет отдельного режима “компактно/просторно” для таблиц (полезно на больших экранах).
...
- [ ] Project switcher: список проектов грузится после login и не ломает layout на mobile.
- [ ] Выбор проекта сохраняется (после reload) и влияет на все страницы (Hosts/Groups/Secrets/Automation).
- [ ] Все API запросы по умолчанию отправляют `X-Project-Id` (если проект выбран), но корректно работают и без него (fallback на доступный проект).
- [ ] Terminal (WS): query param `project_id` соответствует выбранному проекту; при неверном/недоступном проекте — понятная ошибка в UI.
- [ ] Runs live logs (SSE): query param `project_id` соответствует выбранному проекту; при смене проекта активные подписки корректно закрываются/переподключаются.
- [ ] Смена проекта: нет “протечки” данных (после переключения не остаются сущности прошлого проекта в списках/карточках).
- [ ] Ошибки 403/404 по проекту: показываются дружелюбно (toast + текст на странице), без бесконечных retries.

## Что дальше

- [ ] Привести локализацию Hosts/filters к единому русскому словарю.
- [ ] Добавить `aria-sort` и улучшить affordance сортировки в заголовках таблиц.
- [ ] Сделать “sticky actions”/якоря для длинных форм Automation.
